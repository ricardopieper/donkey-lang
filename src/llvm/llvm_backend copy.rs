                    ...
                    for node in block.nodes.iter() {
                        match node {
                            ...
                            MIRBlockNode::Assign {
                                ...
                                root expression is a dereference, we generate a store instead of a load.
                                */
    ...
    pub fn compile_expr_load(
        ...
        match expression {
            HIRExpr::Literal(literal, literal_type, ..) => {
                match literal {
                    LiteralHIRExpr::Integer(i) => {
                        //@TODO I highly doubt this works 
                    ...
                    LiteralHIRExpr::String(s) => {
                        ...
                        str_alloc.into()
                        //self.builder.build_load(str_alloc, "loaded_str").into()
                    }
                    LiteralHIRExpr::None => todo!("none not implemented in llvm"),
                }
            }
            HIRExpr::Variable(name, ..) => {
                let ptr = *symbol_table.get(&(*name, current_scope)).unwrap();
                ptr.into()
                //self.builder.build_load(ptr, "loaded_variable").into()
            }
                ...
                let val = match op.0 {
                    ...
                    Operator::GreaterEquals => {
                        if lhs_type == types.i32 || lhs_type == types.i64 {
                            ...
                            panic!(
                                "Not implemented >= comparison for type {}",
                                lhs_type.as_string(self.type_db)
                            )
                        }
                    }
            ...
            HIRExpr::MemberAccess(obj, member, _, _) => {
                ...
                let llvm_expr = llvm_expr.into_pointer_value();
