...
struct BuiltinArrayType<'ctx> {
    base_field: u64,
    length_field: u64,
    self_type: StructType<'ctx>,
    item_type: AnyTypeEnum<'ctx>,
    item_byte_size: Bytes,
}
impl<'ctx> BuiltinArrayType<'ctx> {
    pub fn new<'codegen_scope>(
        array_type_constructor: &MonoType,
        codegen: &mut CodeGen<'codegen_scope, 'ctx>,
    ) -> Self {
        let llvm_type = codegen.make_llvm_type(array_type_constructor.get_ctor_id());

        let mut base_type_idx = None;
        let mut length_type_idx = None;
        let mut fields: Vec<BasicTypeEnum> = vec![];

        let MonoType::Application(ctor, _) = array_type_constructor else {
            panic!("Expected array iterator application type")
        };

        let ctor = codegen.type_db.find(*ctor);
        //let mut item_type = None;
        for (idx, field) in ctor.fields.iter().enumerate() {
            if field.name == InternedString::new("base") {
                base_type_idx = Some(idx);
            } else if field.name == InternedString::new("length") {
                length_type_idx = Some(idx);
            }
            let field_type = codegen.make_llvm_type(field.field_type.get_ctor_id());

            fields.push(field_type.try_into().unwrap());
        }

        //TODO: get the base ptr<T> type and get the type inside T
        //this is a hack, hopefully works
        let array_item_type = &ctor.find_field("base".into())
          .unwrap().field_type.try_get_type_argument(0).unwrap();

        let llvm_array_item_type = codegen.make_llvm_type(array_item_type.get_ctor_id());

        let item_byte_size = codegen
            .type_db
            .find(array_item_type.get_ctor_id())
            .compute_size(codegen.type_db);

        log!("BYTE SIZE OF ARRAY ITEM: {} {:?}", item_byte_size, array_item_type);


        let result = BuiltinArrayType {
            base_field: base_type_idx.unwrap() as u64,
            length_field: length_type_idx.unwrap() as u64,
            self_type: llvm_type.into_struct_type(),
            item_byte_size,
            item_type: llvm_array_item_type.into()
        };

        result
    }

    pub fn set_length<'codegen_scope>(
        &mut self,
        codegen: &mut CodeGen<'codegen_scope, 'ctx>,
        obj: PointerValue<'ctx>,
        length: u64,
    ) {
        let length_field = codegen
            .builder
            .build_struct_gep(
                self.self_type,
                obj,
                self.length_field as u32,
                &codegen.make_temp("array_length_field"),
            )
            .unwrap();
    ...
    pub fn set_array<'codegen_scope>(
        ...
        let base_field = codegen
            .builder
            .build_struct_gep(
                self.self_type,
                obj,
                self.base_field as u32,
                &codegen.make_temp("array_base_field"),
            )
            .unwrap();
...
impl<'ctx, 'codegen_scope> CodeGen<'codegen_scope, 'ctx> {
    ...
    pub fn make_llvm_type(&mut self, instance: TypeConstructorId) -> AnyTypeEnum<'ctx> {
        ...
        log!("Making LLVM type for {:?}", instance);
        ...
        match struct_name {
            Some(struct_name) => {
                ...
                for (i, param) in function.get_param_iter().enumerate() {
                    param.set_name(parameters[i].name.as_ref());
                }
            ...
            None => {
                let function =
                    self.module
                        .add_function(function_name.as_ref(), function_type, linkage);
    ...
    pub fn generate_for_top_lvl(&mut self, node: &MIRTopLevelNode) {
        match node {
            MIRTopLevelNode::DeclareFunction {
                function_name,
                parameters,
                        ...
                        for (i, item) in items.iter().enumerate() {
                            ...
                            let gep = unsafe {
                                self.builder.build_in_bounds_gep(
                                    ...
                                    allocated_base,
                                    &[index],
                                    &self.make_temp("arr_store_at_idx"),
                                )
                            }
                            .unwrap();
                ...
                match lvalue {
                    ...
                    MIRExprLValue::MemberAccess(obj, member, _, _) => {
                        ...
                        let (index, field) = type_ctor
                            ...
                            .iter()
                            .enumerate()
                            .find(|(_index, field)| field.name == *member)
                            .unwrap();
                        ...
                        let field_ptr = self
                            ...
                            .unwrap();
                        let loaded = self.load_any(field_ptr, field_ty, field.name.as_ref());
                        LlvmRValue(loaded)
                    }
        ...
        if *ctor == self.type_db.common_types.ptr {
            ...
            if method_name == "__index_ptr__" {
                ...
                let gep =
                    unsafe {
                        match pointee {
                            AnyTypeEnum::ArrayType(ty) => self.builder.build_in_bounds_gep(
                                ty,
                                self_value,
                                &[index_arg],
                                "self_at_index",
                            ),
                            AnyTypeEnum::FloatType(ty) => self.builder.build_in_bounds_gep(
                                ty,
                                self_value,
                                &[index_arg],
                                "self_at_index",
                            ...
                            AnyTypeEnum::IntType(ty) => self.builder.build_in_bounds_gep(
                                ty,
                                self_value,
                                &[index_arg],
                                "self_at_index",
                            ),
                            AnyTypeEnum::PointerType(ty) => self.builder.build_in_bounds_gep(
                                ty,
                                self_value,
                                &[index_arg],
                                "self_at_index",
                            ),
                            AnyTypeEnum::StructType(ty) => self.builder.build_in_bounds_gep(
                                ty,
                                self_value,
                                &[index_arg],
                                "self_at_index",
                            ),
                            AnyTypeEnum::VectorType(ty) => self.builder.build_in_bounds_gep(
                                ty,
                                self_value,
                                &[index_arg],
                                "self_at_index",
                            ),
                            AnyTypeEnum::ScalableVectorType(ty) => self
                                .builder
                                .build_in_bounds_gep(ty, self_value, &[index_arg], "self_at_index"),
                            AnyTypeEnum::VoidType(_ty) => {
            ...
            if let Some(f) = self.functions.get(&method_name) {
                let call_site_val = self.builder.build_call(*f, &llvm_args, &self.make_temp(""));
                LlvmRValue(
                ...
                let call_site_val =
                    self.build_function_call_and_enqueue_codegen(&method_name, llvm_args);
                LlvmRValue(
    ...
    pub fn compile_expr_lvalue(
        ...
        match expression {
            MIRExpr::RValue(rvalue) => {
                match rvalue {
                    MIRExprRValue::Literal(literal, _ty, _) => {
                        if let LiteralMIRExpr::String(s) = literal {
                            ...
                            let (str_alloc, _) = self.build_string_literal(s.as_ref());
                            LlvmLValue(str_alloc, llvm_str_type)
                        } else {
                            panic!("invalid lvalue {expression:?}")
                        }
                    }
                    MIRExprRValue::BinaryOperation(_lhs, _op, _rhs, _ty, _) => {
                        panic!("invalid lvalue {expression:?}")
                    }
                    MIRExprRValue::MethodCall(.., _ty, _) => {
                        panic!("invalid lvalue {expression:?}")
                    }
                    MIRExprRValue::FunctionCall(.., _ty, _, _) => {
                        panic!("invalid lvalue {expression:?}")
                    }
                    MIRExprRValue::Ref(expr, ..) => {
                        //you can only create a pointer to something that is an lvalue (i.e. has a memory location)
                    ...
                    MIRExprRValue::Array(_, _, _) => {
                        panic!("invalid lvalue {expression:?}")
                    }
                    MIRExprRValue::StructInstantiate(_ty, _) => {
                        panic!("invalid lvalue {expression:?}")
                    }
                    MIRExprRValue::Cast(_expr, _casted_type, _) => {
                        panic!("invalid lvalue {expression:?}")
                    }
                    MIRExprRValue::TypeVariable { type_variable, .. } => {
                        let type_data_type = self.type_db.find_by_name("TypeData".into()).unwrap();
    ...
    fn build_string_literal(&mut self, s: &str) -> (PointerValue<'ctx>, AnyTypeEnum<'_>) {
    ...
    fn make_temp(&mut self, ty: &str) -> String {
        let ret = format!("{ty}_{}", self.next_temporary);
    ...
    fn get_mangled_type_name(&mut self, ty: &TypeConstructorId) -> InternedString {
        ...
        *self.mangled_name_cache.get(ty).unwrap()
    }
    ...
    fn get_method_name_type_id(
        ...
        self.get_llvm_method_name(&type_id, method_name)
    }
}

type FunctionSymbolTable<'llvm_ctx> = DeterministicMap<
    (InternedString, ScopeId),
    (PointerValue<'llvm_ctx>, BasicTypeEnum<'llvm_ctx>),
>;
...
fn build_function_symbol_table<'mir, 'function_layout, 'ctx>(
    ...
    let mut symbol_table = DeterministicMap::<
        (InternedString, ScopeId),
        (PointerValue<'ctx>, BasicTypeEnum<'ctx>),
    >::new();
    for block in body.iter() {
...
pub fn generate_llvm(
    ...
    let type_data_type = codegen.type_db.find_by_name("TypeData".into()).expect("TypeData struct not found");
    ...
    for _mir_node in mir_top_level_nodes {
