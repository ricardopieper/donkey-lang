...
use inkwell::targets::{TargetMachine, InitializationConfig, Target, RelocMode, CodeModel, FileType};
use inkwell::types::{BasicMetadataTypeEnum, BasicTypeEnum, FunctionType, BasicType, AnyType, AnyTypeEnum};
use inkwell::OptimizationLevel;
use inkwell::values::{AnyValueEnum, FloatValue, FunctionValue, PointerValue, BasicValue, BasicValueEnum, IntValue};

use crate::ast::lexer::Operator;
use crate::compiler::layouts::FunctionLayout;
use crate::llvm;
use crate::semantic::hir::HIRExpr;
use crate::semantic::mir::{MIRTypedBoundName, MIRBlockNode, BlockId, ScopeId, MIRBlock, MIRScope, MIRBlockFinal};
...
impl<'codegen_scope, 'ctx> CodeGen<'codegen_scope, 'ctx> {
    ...
    pub fn make_llvm_type(
        &mut self,
        instance: TypeInstanceId) -> AnyTypeEnum<'ctx> {
        ...
        } else if instance == self.type_db.common_types.i32 || instance == self.type_db.common_types.u32 {
             self.context.i32_type().into()
        } else if instance == self.type_db.common_types.i64 || instance == self.type_db.common_types.u64 {
    ...
    pub fn compile_function_signature(&mut self,
        function_name: &str,
        parameters: &[MIRTypedBoundName],
        return_type: TypeInstanceId) -> FunctionValue<'ctx> {
    ...
    fn create_variable_stack_alloc(&mut self, 
        function: FunctionValue<'ctx>, var_name: &str, 
        var_type: TypeInstanceId, scope: Option<ScopeId>) -> (String, PointerValue<'ctx>) {

        //we create a new builder here because we want to ensure we create the variables in the very beginning
        let builder = self.context.create_builder();

        let first_block = function.get_first_basic_block().expect("Function has no entry basic block");
    ...
    pub fn generate_for_top_lvl(
        ...
        match node {
            MIRTopLevelNode::DeclareFunction {
                ...
                let function_layout = crate::compiler::layouts::generate_function_layout(
                    &scopes,
                    self.type_db
                );
                let function_signature = self.compile_function_signature(&function_name, parameters, *return_type);

                for param in parameters.iter() {
                    self.create_variable_stack_alloc(function_signature, &param.name, param.type_instance, None);
                } 
                ...
                for block in body.iter() {
                    ...
                    for MIRTypedBoundName{type_instance, name} in block_scope.boundnames.iter() {
                        let (llvm_name, ptr) = self.create_variable_stack_alloc(
                            function_signature, &name, *type_instance, Some(block.scope));                        
                ...
                */
                ...
                let symbol_table = build_function_symbol_table(body, scopes, &function_layout, llvm_variables);
                

                let mut llvm_basic_blocks = vec![];
                //foreach block, generate the blocks
                for (i, _) in body.iter().enumerate() {
                    let block = self.context.append_basic_block(function_signature, &format!("block_{i}"));
                   ...
                   for node in block.nodes.iter(){
                        ...
                        match node {
                            MIRBlockNode::Assign { path, expression, .. } => {
                                ...
                                let expr_compiled = self.compile_expr(
                                    block.scope,
                                    &symbol_table,
                                    expression);
                                let ptr = *symbol_table.get(&(path[0].as_str(), block.scope)).unwrap();
                    ...
                    match &block.finish {
                        MIRBlockFinal::If(expr, true_branch, false_branch, _) => {
                            let expr_compiled = self.compile_expr(block.scope, &symbol_table, &expr);
                            self.builder.build_conditional_branch(expr_compiled.into_int_value(), 
                            llvm_basic_blocks[true_branch.0], 
                            llvm_basic_blocks[false_branch.0]);
                        },
                        MIRBlockFinal::GotoBlock(block) =>{
                            self.builder.build_unconditional_branch(llvm_basic_blocks[block.0]);
                        },
                ...
                self.builder.build_unconditional_branch(llvm_basic_blocks[0]);

                function_signature.verify(true);
                self.fpm.run_on(&function_signature);
                function_signature.print_to_stderr();
    ...
    pub fn compile_expr(&mut self, current_scope: ScopeId, symbol_table: &FunctionSymbolTable<'_, 'ctx>, expression: &HIRExpr<TypeInstanceId, Checked>) -> BasicValueEnum<'ctx> {
                ...
                match literal {
                    ...
                    crate::semantic::hir::LiteralHIRExpr::Float(f) => {
                        if types.f32  == *literal_type {
                            self.context.f32_type().const_float(f.0 as f32 as f64).into()
                    ...
                    crate::semantic::hir::LiteralHIRExpr::Boolean(b) => {
                        self.context.bool_type().const_int(if *b {1} else {0}, false).into()
                    },
                    crate::semantic::hir::LiteralHIRExpr::String(_) => todo!("string literals not implemented in llvm"),
            ...
            HIRExpr::BinaryOperation(lhs, op, rhs, _, ..) => {
                ...
                match *op {
                    ...
                    Operator::BitShiftLeft => {
                        if lhs_type.is_integer(self.type_db) {
                            self.builder.build_left_shift(lhs.into_int_value(), rhs.into_int_value(), &self.make_temp("int")).into()
                        } else {
                    ...
                    Operator::BitShiftRight => {
                        if lhs_type.is_integer(self.type_db) {
                            self.builder.build_right_shift(lhs.into_int_value(), rhs.into_int_value(), false, &self.make_temp("int")).into()
                        } else {
                            panic!("Cannot >> in non-int type")
                        }
                    }
                    ...
                    Operator::Equals => {
                        if lhs_type.is_integer(self.type_db) || lhs_type == types.bool {
                            self.builder.build_int_compare(
                                ...
                                lhs.into_int_value(), rhs.into_int_value(), &self.make_temp("int")).into()
                        } else if lhs_type.is_float(self.type_db) {
                            self.builder.build_float_compare(
                                inkwell::FloatPredicate::OEQ,
                                lhs.into_float_value(), rhs.into_float_value(), &self.make_temp("float")).into()
                        } else {
                            panic!("Not implemented Equals for type {}", lhs_type.as_string(self.type_db))
                        }
                    },
                    Operator::NotEquals => {
                        if lhs_type.is_integer(self.type_db) || lhs_type == types.bool {
                            ...
                            panic!("Not implemented Not Equals for type {}", lhs_type.as_string(self.type_db))
                        }
                    }
                    Operator::Or =>  {
                        if lhs_type.is_integer(self.type_db) || lhs_type == types.bool {
                            ...
                            panic!("Not implemented OR for type {}", lhs_type.as_string(self.type_db))
                        }
                    }
                    Operator::And => {
                        if lhs_type.is_integer(self.type_db) || lhs_type == types.bool {
                            ...
                            panic!("Not implemented And for type {}", lhs_type.as_string(self.type_db))
                        }
                    }
                    Operator::Xor => {
                        if lhs_type.is_integer(self.type_db) || lhs_type == types.bool {
                            ...
                            panic!("Not implemented Xor for type {}", lhs_type.as_string(self.type_db))
                        }
                    }
                    Operator::Greater => {
                        ...
                        if lhs_type == types.i32 || lhs_type == types.i64 {
                            ...
                            self.builder.build_float_compare(
                                ...
                                lhs.into_float_value(), rhs.into_float_value(), &self.make_temp("int")).into()
                        } else {
                            panic!("Not implemented Greater Than comparison for type {}", lhs_type.as_string(self.type_db))
                        }
                    }
                    Operator::GreaterEquals => {
                        if lhs_type == types.i32 || lhs_type == types.i64 {
                            ...
                            panic!("Not implemented Greater Than Or Equals comparison for type {}", lhs_type.as_string(self.type_db))
                        }
                    },
                    Operator::Less => {
                        if lhs_type == types.i32 || lhs_type == types.i64 {
                            ...
                            panic!("Not implemented Less Than comparison for type {}", lhs_type.as_string(self.type_db))
                        }
                    },
                    Operator::LessEquals => {
                        if lhs_type == types.i32 || lhs_type == types.i64 {
                            ...
                            panic!("Not implemented Less Than Or Equals comparison for type {}", lhs_type.as_string(self.type_db))
                        }
                    }
            ...
            HIRExpr::UnaryExpression(op, expr, ..) => {
                ...
                match op {
                    ...
                    Operator::Minus =>{
                        ...
                        if lhs_type.is_integer(self.type_db) || lhs_type == types.bool {
                            self.builder.build_int_neg(compiled.into_int_value(), &self.make_temp("int")).into()
                        } else if lhs_type.is_float(self.type_db) {
                            self.builder.build_float_neg(compiled.into_float_value(), &self.make_temp("float")).into()
                        } else {
                            panic!("Not implemented")
                        }
    ...
    fn make_temp(&mut self, ty: &str) -> String {
        let ret = format!("tmp_{}{}", ty, self.next_temporary);
...
fn build_function_symbol_table<'mir, 'f,  'ctx>(
    ...
    llvm_variables: HashMap<String, PointerValue<'ctx>>) -> FunctionSymbolTable<'f, 'ctx> {
    ...
    for block in body.iter() {
        ...
        for (name, (_, declaring_scope)) in variables_on_scope {
            ...
            let variable_llvm = llvm_variables.get(&llvm_name).expect("Should find the variable here");
...
pub fn generate_llvm(
    ...
    codegen.generate_for_top_lvl(mir_top_level_nodes.get(0).unwrap());
    //}
    ...
    unsafe {
        let out = compiled_fn.call();
        println!("Result: {}", out);
    }

    let elapsed = now.elapsed();
    println!("Code took {}ns to run", elapsed.as_nanos());
...
impl FooFactory {
    ...
    fn make_foo(&self) -> Foo {
        self.fac_impl.make_foo()
    }
}

struct FooFactoryImpl {}

impl FooFactoryImpl {
    fn make_foo<'foo_factory> (&self) -> Foo<'foo_factory> {
        Foo {
...
struct Foo<'foo_factory> {
    _phantom: std::marker::PhantomData<&'foo_factory FooFactory>
}
...
impl Drop for FooFactory {
    fn drop(&mut self) {
        
    }
}

struct MyObject<'obj, 'foo_factory> {
    factory: &'foo_factory FooFactory,
    foo: &'obj Foo<'foo_factory>
}

impl<'obj, 'foo_factory> MyObject<'obj, 'foo_factory> {
    fn do_work(&'foo_factory self, s: &str) {

    }
}

fn make_compiler_complain() {

    let fac = FooFactory::new();
    let foo = fac.make_foo();
