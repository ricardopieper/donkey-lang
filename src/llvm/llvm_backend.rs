...
use crate::types::type_constructor_db::{Bytes, TypeConstructor, TypeConstructorDatabase, TypeConstructorId, TypeKind};
...
pub enum LlvmExpression<'ctx> {
    ...
    LValue(PointerValue<'ctx>, AnyTypeEnum<'ctx>),
    LoadedLValue(BasicValueEnum<'ctx>, PointerValue<'ctx>, AnyTypeEnum<'ctx>),
}

impl<'ctx> LlvmExpression<'ctx> {


    pub fn load_any<'c>(builder: &Builder<'c>, ptr: PointerValue<'c>, ty: AnyTypeEnum<'c>, name: &str) -> BasicValueEnum<'c> {
        match ty {
            AnyTypeEnum::ArrayType(t) => builder.build_load(t, ptr, name),
            AnyTypeEnum::FloatType(t) =>builder.build_load(t, ptr, name),
            AnyTypeEnum::FunctionType(t) => panic!("load lvalue of type function"),
            AnyTypeEnum::IntType(t) =>builder.build_load(t, ptr, name),
            AnyTypeEnum::PointerType(t) => builder.build_load(t, ptr, name),
            AnyTypeEnum::StructType(t) => builder.build_load(t, ptr, name),
            AnyTypeEnum::VectorType(t) => builder.build_load(t, ptr, name),
            AnyTypeEnum::ScalableVectorType(t) => builder.build_load(t, ptr, name),
    ...
    pub fn load_if_lvalue(self, builder: &Builder<'ctx>) -> LlvmExpression<'ctx> {
        match self {
            Self::LValue(ptr, ty) => Self::LoadedLValue(
                LlvmExpression::load_any(builder, ptr, ty, "loaded_ptr"), ptr, ty),
            Self::RValue(val) => Self::RValue(val),
            Self::LoadedLValue(val, ptr, ty) => Self::LoadedLValue(val, ptr, ty),
        }
    }
    ...
    pub fn expect_rvalue(self) -> BasicValueEnum<'ctx> {
        match self {
            ...
            Self::RValue(val) => val,
            Self::LoadedLValue(val, ..) => val,
        }
    }
    pub fn expect_lvalue(self) -> (PointerValue<'ctx>, AnyTypeEnum<'ctx>) {
        match self {
            Self::LValue(ptr, ty, ..) => (ptr, ty),
            Self::RValue(val) => panic!("Expected an lvalue but got ${val:?}, address is unknown"),
            Self::LoadedLValue(_, ptr, ty, ..) => (ptr, ty),
        }
    }
}
...
impl<'codegen_scope, 'ctx> CodeGen<'codegen_scope, 'ctx> {
    ...
    pub fn register_top_lvl(&mut self, node: &MIRTopLevelNode) {
        match node {
            MIRTopLevelNode::DeclareFunction {
                ...
                self.create_function(*function_name, parameters, type_table[return_type].mono.get_ctor_id(), false, false, type_table);
            }
    ...
    fn add_type_data(&mut self, type_instance: TypeConstructorId) {
        ...
        let name = self.build_string_struct_value_for_globals(StringInterner::get().borrow(instance.name));
        let size = self
            .context
            .i64_type()
    ...
    pub fn generate_for_top_lvl<'source>(&mut self, node: &'source MIRTopLevelNode) {
        match node {
            MIRTopLevelNode::DeclareFunction {
                ...
                let function_layout =
                    generate_function_layout(scopes, self.type_db, type_table);
                ...
                let mut llvm_variables: HashMap<InternedString, (PointerValue<'_>, BasicTypeEnum<'_>)> = HashMap::new();
                ...
                */
                ...
                self.builder
                    .build_unconditional_branch(llvm_basic_blocks[0]).unwrap();
                    ...
                    for node in block.nodes.iter() {
                        match node {
                            MIRBlockNode::Assign {
                                ...
                                let lvalue = self.compile_lvalue(path, &symbol_table, block.scope, type_table);
                                let (ptr, _) = lvalue.expect_lvalue();
                                let value_to_write = self
                                    .compile_expr(block.scope, &symbol_table, expression, type_table)
                            ...
                            MIRBlockNode::FunctionCall { function, args, .. } => {
                                ...
                                let llvm_args = args
                                    .iter()
                                    .map(|x| {
                                        self.compile_expr(block.scope, &symbol_table, x, type_table)
                                            .load_if_lvalue(self.builder)
                                            .expect_rvalue()
                                            .into()
                                    })
                            ...
                            MIRBlockNode::MethodCall {
                                ...
                                let method_name =
                                    self.get_method_name_type_id(object_type.mono.get_ctor_id(), *method_name);
                                let mut llvm_args: Vec<BasicMetadataValueEnum> = vec![];
                                let self_value = self
                                    .compile_expr(block.scope, &symbol_table, object, type_table)
                                    .load_if_lvalue(self.builder)
                                    .expect_rvalue()
                                    .into();
                                //TODO deduplicate this code
                                ...
                                if let Some(f) = self.functions.get(&method_name) {
                                    self.builder.build_call(*f, &llvm_args, &self.make_temp("")).unwrap();
                                } else {
                    ...
                    match &block.finish {
                        MIRBlockFinal::If(expr, true_branch, false_branch, _) => {
                            let expr_compiled = self
                                .compile_expr(block.scope, &symbol_table, expr, type_table)
                                .load_if_lvalue(self.builder)
                                .expect_rvalue();
                        ...
                        MIRBlockFinal::GotoBlock(block) => {
                            self.builder
                                .build_unconditional_branch(llvm_basic_blocks[block.0]).unwrap();
                        }
            ...
            MIRTopLevelNode::IntrinsicOrExternalFunction {
                ...
                if function_name.as_ref().starts_with("reinterpret_ptr") && !is_external {
                    ...
                    let casted = self.builder.build_pointer_cast(origin, target, "casted").unwrap();
    ...
    fn build_function_call_and_enqueue_codegen(
        ...
        if let Some(f) = self.functions.get(function) {
            self.builder
                .build_call(*f, &llvm_args, &self.make_temp("fcall_ret")).unwrap()
        } else {
            //
    ...
    pub fn compile_expr(
        ...
        match expression {
            MIRExpr::RValue(rvalue) => self.compile_rvalue(rvalue, current_scope, symbol_table, type_table),
            MIRExpr::LValue(lvalue) => self.compile_lvalue(lvalue, symbol_table, current_scope, type_table),
        }
    }
    ...
    fn compile_rvalue(
        ...
        match rvalue {
            MIRExprRValue::Literal(literal, ty, _) => self.build_literal(literal, types, &type_table[*ty].mono.get_ctor_id()),
            MIRExprRValue::BinaryOperation(lhs, op, rhs, _ty, _) => {
                self.build_binary_expr(lhs, rhs, current_scope, symbol_table, op, types, type_table)
            }
            MIRExprRValue::MethodCall(object, method_name, args, _ty, _) => {
                let method_name = self.get_method_name_type_id(type_table[object.get_type()].mono.get_ctor_id(), *method_name);
                let mut llvm_args: Vec<BasicMetadataValueEnum> = vec![];
                let self_value = self
                    .compile_expr(current_scope, &symbol_table, object, type_table)
                    .load_if_lvalue(self.builder)
                    .expect_rvalue()
                    .into();
                //TODO deduplicate this code
                let call_args: Vec<BasicMetadataValueEnum> = args
                    .iter()
                    .map(|x| {
                        self.compile_expr(current_scope, &symbol_table, x, type_table)
                            .load_if_lvalue(self.builder)
                            .expect_rvalue()
                            .into()
                    })
            ...
            MIRExprRValue::FunctionCall(function_expr, arg_exprs, _ty, _, _) => {
                ...
                let llvm_args = arg_exprs
                    ...
                    .map(|x| {
                        let expr_compiled = self
                            .compile_expr(current_scope, symbol_table, x, type_table)
                            .load_if_lvalue(self.builder)
                            //.load(self.builder);
                            ;
            ...
            MIRExprRValue::Ref(expr, ..) => {
                ...
                let ptr = self.compile_lvalue(expr, symbol_table, current_scope, type_table);
                let (r, p) = ptr.expect_lvalue();
                LlvmExpression::RValue(r.into())
            }
            MIRExprRValue::UnaryExpression(op, expr, _, _) => {
                let lhs_type = expr.get_type();
                let compiled = self
                    .compile_expr(current_scope, symbol_table, expr, type_table)
                    .load_if_lvalue(self.builder)
                    .expect_rvalue();
            ...
            MIRExprRValue::StructInstantiate(ty, _) => {
                ...
                let llvm_struct = self
                    .builder
                    .build_alloca(llvm_str_type.into_struct_type(), &self.make_temp("struct")).unwrap();
                LlvmExpression::LValue(llvm_struct, llvm_str_type)
            }
            MIRExprRValue::Cast(expr, casted_type, _) => {
                ...
                let llvm_casted_type = self.make_llvm_type(type_table[casted_type].mono.get_ctor_id());

                //generate a numeric cast, i.e. i32 -> i64, f32 -> f64, i32 -> f64, etc

                let casted: BasicValueEnum = if self.type_db.is_integer(type_table[expr.get_type()].mono.get_ctor_id()) {
                    if self.type_db.is_integer(type_table[casted_type].mono.get_ctor_id()) {
                        self.builder
                            .build_int_cast(
                                expr_compiled.into_int_value(),
                            ...
                            .unwrap()
                            .into()
                    } else if self.type_db.is_float(type_table[casted_type].mono.get_ctor_id()){
                        self.builder
                ...
                } else if self.type_db.is_float(type_table[expr.get_type()].mono.get_ctor_id()) {
                    if self.type_db.is_integer(type_table[casted_type].mono.get_ctor_id())  {
                        self.builder
    ...
    fn compile_lvalue(
        ...
        match lvalue {
            MIRExprLValue::Variable(name, _, _) => {
                ...
                match symbol_table.get(&(*name, current_scope)) {
                    Some((ptr, ty)) => {
                        ...
                        LlvmExpression::LValue(*ptr, as_any)
                    },
                    None => {
                        panic!(
            ...
            MIRExprLValue::MemberAccess(obj, member, _, _) => {
                let (llvm_expr, llvm_expr_ty) = self
                    .compile_expr(current_scope, symbol_table, obj, type_table)
                    .expect_lvalue();

                let type_ctor = self.type_db.find(type_table[obj.get_type()].mono.get_ctor_id());
                let (index, field) = type_ctor.fields.iter().enumerate().find(|(index, field)| field.name == *member).unwrap();
                ...
                let field_ptr = self
                    .builder
                    .build_struct_gep(llvm_expr_ty.into_struct_type(), llvm_expr, index as u32, &self.make_temp("var"))
                    .unwrap();

                LlvmExpression::LValue(field_ptr, field_ty)
            }
            MIRExprLValue::Deref(expr, _, _) => {
                ...
                let type_ctor = self.make_llvm_type(type_table[expr.get_type()].mono.get_ctor_id());

                let expr = self.compile_expr(current_scope, symbol_table, expr, type_table);
                match expr {
                    LlvmExpression::RValue(basic) => {
                        let as_ptr = basic.into_pointer_value();
                        LlvmExpression::LValue(as_ptr, type_ctor)
                    }
                    LlvmExpression::LValue(ptr, ty)
                    | LlvmExpression::LoadedLValue(_, ptr, ty) => {

                        let loaded_ptr =
                            LlvmExpression::load_any(self.builder, ptr, ty, "deref_load_ptr").into_pointer_value();

                        let ptr_ty =  Context::ptr_type(self.context, AddressSpace::default()).into();

                        let deref_ptr =
                            LlvmExpression::load_any(self.builder, loaded_ptr, ptr_ty, "deref");

                        LlvmExpression::LoadedLValue(deref_ptr, loaded_ptr, ptr_ty)
                    }
                }
            }
        }
    ...
    fn build_binary_expr(
        ...
        let lhs = self
            .compile_expr(current_scope, symbol_table, lhs, type_table)
            .load_if_lvalue(self.builder)
            .expect_rvalue();

        let rhs = self
            .compile_expr(current_scope, symbol_table, rhs, type_table)
            .load_if_lvalue(self.builder)
            .expect_rvalue();
    ...
    fn build_literal(
        ...
        match literal {
            ...
            LiteralMIRExpr::String(s) => {
                ...
                LlvmExpression::LValue(str_alloc, llvm_str_type).load_if_lvalue(self.builder)
    ...
    fn build_string_literal(&mut self, s: &str) -> (PointerValue<'ctx>, AnyTypeEnum) {
        ...
        let global_str = self
            .builder
            .build_global_string_ptr(&null_terminated, ".str").unwrap();
        let buf_val = global_str.as_pointer_value();

        //allocate struct
        let str_alloc = self
            .builder
            .build_alloca(llvm_str_type.into_struct_type(), "_str").unwrap();
        {
            let ptr_to_buf = self
                .builder
    ...
    fn get_mangled_type_name(&mut self, ty: &TypeConstructorId) -> InternedString {
        ...
        let mangled = name.replace("[", "__T__");
        self.mangled_name_cache.insert(*ty, mangled.into());
    ...
    fn generate_ptr_write_intrinsic(&mut self, type_data: &TypeConstructor) {
        ...
        let llvm_function =
            self.create_function(method_name.into(), &parameters, return_type.get_ctor_id(), false, false, &type_table);
        ...
        let store_address = unsafe {
            match self_llvm_type {
                AnyTypeEnum::ArrayType(t) => self.builder.build_in_bounds_gep(t, self_arg, &[index_arg], "self_at_index"),
                AnyTypeEnum::FloatType(t) =>  self.builder.build_in_bounds_gep(t, self_arg, &[index_arg], "self_at_index"),
                AnyTypeEnum::FunctionType(t) => panic!("Cannot store in function ptr"),
                AnyTypeEnum::IntType(t) =>  self.builder.build_in_bounds_gep(t, self_arg, &[index_arg], "self_at_index"),
                AnyTypeEnum::PointerType(t) =>  self.builder.build_in_bounds_gep(t, self_arg, &[index_arg], "self_at_index"),
                AnyTypeEnum::StructType(t) =>  self.builder.build_in_bounds_gep(t, self_arg, &[index_arg], "self_at_index"),
                AnyTypeEnum::VectorType(t) =>  self.builder.build_in_bounds_gep(t, self_arg, &[index_arg], "self_at_index"),
                AnyTypeEnum::ScalableVectorType(t) =>  self.builder.build_in_bounds_gep(t, self_arg, &[index_arg], "self_at_index"),
                AnyTypeEnum::VoidType(t) =>  panic!("Cannot store in void ptr"),
            }
        }.unwrap();
        self.builder.build_store(store_address, element_arg).unwrap();
...
type FunctionSymbolTable<'llvm_ctx> = HashMap<(InternedString, ScopeId), (PointerValue<'llvm_ctx>, BasicTypeEnum<'llvm_ctx>)>;
...
fn build_function_symbol_table<'mir, 'function_layout, 'ctx>(
    ...
    let mut symbol_table = HashMap::<(InternedString, ScopeId),  (PointerValue<'ctx>, BasicTypeEnum<'ctx>)>::new();
    for block in body.iter() {
