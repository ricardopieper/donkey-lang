...
impl Memory {
    ...
    pub fn read(&self, address: u32, len: u32) -> (&[u8], bool, u32, u32) {
        ...
        if index + len > PAGE_SIZE as u32 {
            return self.read_page_fault(address, len);
        } else {
            return self.read_normal(address, len);
        }
    }
    ...
    pub fn read_single(&self, address: u32) -> u8 {
        ...
        unsafe { *page.as_ref().unwrap_unchecked().get_unchecked((address & 0x0000ffff) as usize)}
    }
    ...
    fn read_normal(&self, address: u32, len: u32) -> (&[u8], bool, u32, u32) {
        ...
        (unsafe {
            ...
            let offsetted = ptr.offset(index as isize);
            &*std::ptr::slice_from_raw_parts(offsetted, len as usize)
        }, false, 0, 0)
    }

    fn get_page<'a>(&'a self, address: u32) -> &Option<Box<Page>> {
        unsafe {
            //mem is always filled
            let address_page = address >> 16;
           
            let ptr = self.mem.get_unchecked(address_page as usize);
            return ptr;
        }
    }


    fn get_page_mut<'a>(&'a mut self, address: u32) -> &mut Option<Box<Page>> {
        unsafe {
            //mem is always filled
            let address_page = address >> 16;
    ...
    fn get_page_and_allocate<'a>(&'a mut self, address: u32) -> Option<&'a mut Page> {
        unsafe {
            //mem is always filled
            let address_page = address >> 16;
    ...
    fn ensure_page_allocated<'a>(&'a mut self, address: u32) {
        unsafe {
            //mem is always filled
            let address_page = address >> 16;
    ...
    fn get_page_read_unsafe<'a>(&'a mut self, address: u32) -> &'a mut Page {
        //mem is always filled
        let address_page = address >> 16;
    ...
    fn get_page_very_safe<'a>(&'a mut self, address: u32) -> &'a mut Page {
    ...
    pub fn copy(&mut self, address_from: u32, address_to: u32, len: u32) {
        ...
        let max_possible_read = 
            read_size_before_fault_from
                .min(read_size_before_fault_to);
        ...
        if len > max_possible_read {
            ...
            self.copy(address_from + read_size, address_from + read_size, remaining)
        }
...
mod tests {
    ...
    pub fn baseline() {
        ...
        for addr in start..end {
            let offset = unsafe {ptr.offset(addr as isize) };
            unsafe {*offset = 9};
        }
        ...
        for _ in 0..1 {
            for addr in start..end {
                let read = unsafe { *ptr.offset(addr as isize) };
                assert_eq!(read, 9u8);
            }
        }
    }
